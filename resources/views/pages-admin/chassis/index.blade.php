...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Data Chassis</h1>
        <a href="/chassis/create"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-200 text-gray-700">
                    @forelse ($chassis as $chassiss)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-center font-medium">{{ $chassiss->nama }}</td>
                            <td class="px-6 py-4 text-center">{{ $chassiss->deskripsi ?? '-' }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="/chassis/{{ $chassiss->id }}"
                                        class="inline-flex items-center justify-center w-9 h-9 rounded-full text-white shadow transition"
                                        style="background-color: #facc15;">
                                        <i class="fas fa-edit text-sm"></i>
                                    </a>
                                    <button onclick="confirmDelete('{{ $chassiss->id }}', '{{ $chassiss->nama }}')"
                                        class="inline-flex items-center justify-center w-9 h-9 rounded-full text-white shadow transition"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-6 text-gray-400 italic">
                                Tidak ada data jenis dump truck.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        function confirmDelete(id, name) {
            Swal.fire({
                title: 'Hapus Jenis?',
                text: `Yakin ingin menghapus jenis "${name}"?`,
                icon: 'warning',
