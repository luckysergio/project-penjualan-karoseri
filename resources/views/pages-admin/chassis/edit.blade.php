...
<div class="max-w-xl mx-auto mt-10">
    <div class="bg-white shadow-lg rounded-xl p-6 sm:p-8">
        <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6 text-center">Edit chassis</h2>
        <form action="{{ url('/chassis/'.$chassis->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700 mb-1">Nama chassis</label>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                    class="block w-full px-4 py-2 text-sm border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition
                    {{ $errors->has('deskripsi') ? 'border-red-500' : 'border-gray-300' }}">{{ old('deskripsi', $chassis->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
