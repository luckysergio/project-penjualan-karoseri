...
@section('content')
    ...
    <div class="max-w-xl mx-auto mt-10">
        <div class="bg-white shadow-lg rounded-xl p-6 sm:p-8">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6 text-center">Form Tambah Pengiriman</h2>
            <form action="/pengiriman" method="POST" class="space-y-5">
                @csrf

                <div>
                    <label for="id_order" class="block text-sm font-medium text-gray-700 mb-1">Order</label>
                    <select name="id_order" id="id_order"
                        ...
                        @foreach ($orders as $order)
                            <option value="{{ $order->id }}" {{ old('id_order') == $order->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                        class="w-full px-4 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="biaya" id="biaya" inputmode="numeric"
                        class="w-full px-4 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500"
                        value="{{ old('biaya') }}" placeholder="Masukkan biaya pengiriman">
                    @error('biaya')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
