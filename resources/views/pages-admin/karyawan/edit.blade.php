...
@section('content')
    ...
    <div class="max-w-2xl mx-auto mt-10">
        <div class="bg-white shadow-lg rounded-xl p-6 sm:p-8">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6 text-center">Form Edit Karyawan</h2>
            <form action="{{ url('/karyawan/' . $karyawan->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
        ...
        {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }}"
                        value="{{ old('name', $karyawan->user->name) }}" placeholder="Masukkan nama">
                    @error('name')
        ...
        {{ $errors->has('email') ? 'border-red-500' : 'border-gray-300' }}"
                        value="{{ old('email', $karyawan->user->email) }}" placeholder="Masukkan email">
                    @error('email')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Password <small>(kosongkan
        ...
        {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }}"
                ...
                <div>
                    ...
                    <input type="text" name="nik" id="nik"
                        ...
                        value="{{ old('nik', $karyawan->nik) }}" placeholder="Masukkan NIK karyawan">
                    @error('nik')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
