...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Data Type Dump</h1>
        <a href="/type/create"
                ...
                <tbody class="divide-y divide-gray-200 text-gray-700">
                    @forelse ($type_dumps as $type)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-center font-medium">{{ $type->nama }}</td>
                            <td class="px-6 py-4 text-center">{{ $type->kapasitas }}</td>
                            <td class="px-6 py-4 text-center">
                                {{ $type->harga ? 'Rp ' . number_format($type->harga, 0, ',', '.') : '-' }}
                            </td>
                            <td class="px-6 py-4 text-center">{{ $type->deskripsi }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <button onclick="confirmDelete('{{ $type->id }}', '{{ $type->nama }}')"
                                        class="inline-flex items-center justify-center w-9 h-9 rounded-full text-white shadow transition"
