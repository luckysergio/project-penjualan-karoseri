...
@section('content')
    ...
    <div class="max-w-xl mx-auto mt-10">
        <div class="bg-white shadow-lg rounded-xl p-6 sm:p-8">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6 text-center">Form Tambah Type Dump</h2>
            <form action="{{ url('/type') }}" method="POST" class="space-y-5">
                @csrf
            ...
            {{ $errors->has('nama') ? 'border-red-500' : 'border-gray-300' }}"
                        value="{{ old('nama') }}" placeholder="Masukkan nama type dump">
                    @error('nama')
            ...
            {{ $errors->has('kapasitas') ? 'border-red-500' : 'border-gray-300' }}"
                        value="{{ old('kapasitas') }}" placeholder="Masukan kapasitas">
                    @error('kapasitas')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
